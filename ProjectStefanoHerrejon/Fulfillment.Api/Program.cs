...
app.MapGet("/", () => 
{
    return Results.Ok("Welcome to Stefano's DEMO!!!");
});

//Get all the inventory
app.MapGet("/inventory", async (FulfillmentDBContext db, ILogger<Program> logger) =>
{
    var inventory = await db.Inventory.ToListAsync();

    if(inventory.Count > 0)
        return Results.Ok(inventory);
    else
        return Results.NotFound(inventory);
});
...
app.MapGet("/orders-all-time", async (IOrderService orderService, CancellationToken ct) =>
{
    ...
    if(report.Count > 0)
        ...
        return Results.NotFound(report);
});
...
//Orders history of single specific client, lookup with id
...
app.MapGet("orders/CountPending", (FulfillmentDBContext db, CancellationToken ct) =>
{
    int count = 0;
    ...
    return  Results.Ok($"Orders Pending : {count}");
});

//Delete Pending orders
app.MapDelete("orders/deletePending", async (FulfillmentDBContext db, CancellationToken ct) =>
{
    int count = 0;
    foreach(var inv in db.Orders)
    {
    ...
    Log.Logger.Information("Pending Orders #:{Count} deleted", count);
...
//Quick Method to create and fulfill a single order
app.MapPost("/orders/create", async (OrderPayLoad orderRequest,IDbContextFactory<FulfillmentDBContext> factory, 
...
//Method to create n orders, and fulfilled them via burst
app.MapPost("/orders/burst", (int n, bool expedited, ISeeder seeder, IServiceScopeFactory scopes, IHostApplicationLifetime lifetime) =>
{
...
app.MapGet("/benchmark-SeqVsBurst", async (int n, IFulfillmentService fs, ISeeder seeder, CancellationToken ct) =>
{
...
//BenchMark for Expedited vs normal orders
app.MapGet("/benchmark-PriorityQueue", async (int n, FulfillmentDBContext db,IFulfillmentService fs, ISeeder seeder, CancellationToken ct) =>
{
    ...
    return Results.Ok(report);//Return results
    
});

//Gets the most n rank ticket based on sales
app.MapGet("/benchmark-Reports", async (int topRank, FulfillmentDBContext db, IFulfillmentService fs, ISeeder seeder, CancellationToken ct) =>
{
    ...
    if (index >= 0)
    {
        ...
        return Results.Ok($"Ticket {product.TicketID} is ranked #{product.Rank} with totalsold : {product.TotalSold}");
    }else return Results.NotFound($"No ticket with rank : {topRank}");
...
public record TopProducts(int TicketID, int TotalSold, int Rank);
