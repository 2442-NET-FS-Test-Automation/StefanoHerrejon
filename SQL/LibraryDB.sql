...
-- Aggreagete function
-- An aggregate functions collapses many rows into one number
-- COUNT() COUNT(*) is different from COUNT(some column) - when you count a specific column, NULLS are ignored
-- SUM() - get the total sum of storered in a column across many rows
-- AVG() - get the average value stored in a column across many rows - will skip nulls
-- MIN() - MAX()
...
SELECT 
    UPPER(LastName) as LastUpper,
    LEN(Email) as EmailLen,
    CONCAT(FirstName,' ',LastName) as FullName,
    DATEDIFF(DAY, JoinedDate, GETDATE()) AS DayAsAMember -- Takes 3 arguments, the datepart (year difference, day differente, etc)
        -- Start Date, End Date
FROM dbo.Member; 
...
-- This is an example of an equity join, we join on an equality comparison
SELECT 
    b.Title,
    c.Name AS CATEGORY
FROM dbo.Book as b 
INNER JOIN dbo.Category as c 
ON c.CategoryId = b.CategoryId -- The join condition
ORDER BY c.Name, b.Title;

-- I want stuff from Authors and Books - I nees to traverse BookAuthor
-- BOOKs with all their authors, throught the join table
SELECT 
    b.Title,
...
-- GROUP BY returns onw row in the result: PER GROUP
-- Aggregates collapses all rows in a group into a single value
-- A bare column, would have many possible values per group

-- LEFT and RIGHT JOIN
-- LEFT JOIN - we want all the records from the left table, and matching records from the right
...
SELECT
    ...
    l.LoanId,
    l.DueDate
FROM dbo.Member m -- Left Table
LEFT JOIN dbo.Loan l on m.MemberId = l.MemberId
ORDER BY m.LastName;
...
SELECT
    ...
    m.LastName
FROM dbo.Member m
LEFT JOIN dbo.Loan AS l ON m.MemberId = l.MemberId
WHERE l.LoanId is NULL;

-- RIGHT JOIN, mirror of left join
-- every book and their loans if it exist
SELECT 
    b.Title,
    l.LoanId
FROM dbo.Loan AS l  
...
-- FULL OUTER vs CROSS
-- FULL OUTIER JOIN - Returns matched rows where they exist, as well as unmatched rows
SELECT 
    b.Title,
    c.Name AS Category
FROM dbo.Book b
...
-- CROSS JOIN - not commmon. Cartesion product
...
-- I want every currently out loan (Loans with a null), with member,  category, and how late the book is
