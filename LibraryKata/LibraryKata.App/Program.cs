...
public class Program
{
    ...
    public static void AdvenceClassDemo()
    {
        Console.WriteLine("\n == Advance classes ==");

        //First, a quick detour, lets interact with the garbage collector = GC
        ...
        repo.Add(LibraryItemFactory.Create(ItemKind.Magazine, "Wired", "Axel", copies:3));
        repo.Add(LibraryItemFactory.Create(ItemKind.Book, "Dune Messias", "Frank Herbert", copies:3));
        repo.Add(LibraryItemFactory.Create(ItemKind.ReferenceBook, "Dune", "Microsoft", copies:3));
        

        //Magazine need a publisher, bit we provided a default valur for the publisher argument in Create()
        //Lets see if it works
        repo.Add(LibraryItemFactory.Create(ItemKind.Magazine, "Wired","Axel", copies:2));
        ...
        //Lest use out Find() method

        List<LibraryItem> byFrankHerbert = catalog.Find(item => item.Author == "Frank Herbert");
        Console.WriteLine($"There are {byFrankHerbert.Count} books by Frank Hernert");
    ...
    {
        Console.WriteLine("Async Http Demo");
        ...
        // I want to fetch the data from OpenLibrary for both ISBNNs
        // I do not want to sit here and type the same code calling the same method for both ISBNs
        // I would end up awaiting two almost identical calls - thats valid but the curriculum says "optimizing async code"

        Task<LibraryItem?>[] fetchedBooks = new Task<LibraryItem?>[isbns.Length];

        //NExt, we loop through the array and call FectchByIdAsync - we use a traditional c-syntax for loop
        //because we care about indexes for this
        for(int i = 0; i < isbns.Length; i++)
        {
            //Notice, this is an async method call - but we didnt await it
        ...
        //foundBok = awaitt client.FetechByIsbnAsync("1234567890123");
        ...
        //This works, but what if we have a null
        //LibraryItem? firstBookFound = foundBooks[0];

        //To be sage, we can use a quicl ternary operator. Like a quick if-else check
        //ternary syntex (some codition to check) truevalue ? falsevalue
        LibraryItem? firstBookFound = foundBooks.Length > 0 ? foundBooks[0]:null;


        //Using WhenAll to do concurrent fetching. If we didnt do this,  and we awaited wery single call one by one
        //Think about the amount of latency we'd be eating
        Console.WriteLine($"Fetched: {firstBookFound?.Describe() ?? "nothing"}");


        //Boxing and Unboxing - mostly depreceated, replaced by generics
        //Sometimes we needed to store valur types on the heap, think of adding an int to a list. Before generics (list<T>)
        //we had ArrayListto acomplish the same thing. Under the hood, an Arraylist coudn't accept value types
        ...
        //ArrayList - deprecated, slow, sus boxing and unboxing
    }
}